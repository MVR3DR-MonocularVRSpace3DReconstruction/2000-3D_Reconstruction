...
from fragment_registration.open3d_utils import join, get_file_list, write_poses_to_log, draw_registration_result_original_color
from fragment_registration.optimize_posegraph import optimize_posegraph_for_refined_scene
...
def multiscale_icp(source, target, voxel_size,
                   max_iter, config, init_transformation=np.identity(4)):
...
def make_posegraph_for_refined_scene(ply_file_names, config):
    ...
    o3d.io.write_pose_graph(config["path_dataset"]+config["template_refined_posegraph"],pose_graph_new)
